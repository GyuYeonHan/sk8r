			...
			const searchPromises = searchableResources.map(async (resourceType) => {
				try {
					const response = await apiClient(
						`/api/search?q=${encodeURIComponent(searchQuery)}&type=${resourceType}&namespace=*`
					);
					if (response.ok) {
...
{#if isOpen}
	...
	<!-- Search Modal -->
	<div
		class="fixed top-20 left-1/2 z-50 w-full max-w-2xl -translate-x-1/2 transform rounded-lg border border-gray-200 bg-white shadow-2xl"
	>
			...
			<div class="max-h-96 overflow-y-auto">
				{#each results as result, index}
					<button
						...
						class="flex w-full items-center justify-between border-b border-gray-100 px-4 py-3 text-left transition-colors last:border-b-0 hover:bg-gray-50"
		...
		<div
			class="flex items-center justify-between border-t border-gray-200 bg-gray-50 px-4 py-3 text-xs text-gray-500"
		>
